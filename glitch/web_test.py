...
def apply_datamosh_effect(image, direction="vertical", melt_region_percentage=30, variation_percentage=10, max_shift=500, selection = None):
    ...
    if not layering_system:
        transparent_base = original_pixels
    else:
        transparent_base = np.zeros((height, width, channels), dtype=np.uint8)
    pixels = transparent_base.copy()
    if selection:
        x, y, w, h = selection
        leak_x = int(w * variation_percentage)
        leak_y = int(h * variation_percentage)

        x = max(0, x - leak_x)
        y = max(0, y - leak_y)
        x_end = min(x + w + 2 * leak_x, width)
        y_end = min(y + h + 2 * leak_y, height)

        if direction == "vertical":
            for x_pos in range(x, x_end):
                # Randomize the melt start and end for each column
                column_melt_start = random.randint(y, y + int((y_end - y) * (1 - melt_region_percentage)))
                column_melt_end = random.randint(column_melt_start + 1, y_end)
                ...
                for y_pos in range(column_melt_start, column_melt_end):
            ...
            for y_pos in range(y, y_end):
                # Randomize the melt start and end for each row
                row_melt_start = random.randint(x, x + int((x_end - x) * (1 - melt_region_percentage)))
                row_melt_end = random.randint(row_melt_start + 1, x_end)
                ...
                for x_pos in range(row_melt_start, row_melt_end):
        ...
        if direction == "vertical":
            for x in range(width):        
                ...
                shift_height = random.randint(1, int(max_shift))
    ...
    for selection in selections:
        ...
        if selection['apply_datamosh_selection']:
            if layering_system:
                ...
                img = apply_datamosh_effect(img, selection['datamosh_selection_direction'], selection['datamosh_selection_melt_region_percentage'], selection['datamosh_selection_variation_percentage'], selection['datamosh_selection_max_shift'], selection_coords)
        if selection['apply_corrupt_selection']:
            if layering_system:
                ...
                img = apply_corrupt_effect(img, selection['corrupt_selection_direction'], selection_coords)
        if selection['apply_melt_selection']:
            if layering_system:
                ...
                img = apply_melt_effect(img, selection['melt_selection_amount'], selection['melt_selection_direction'], selection_coords)
...
def process():
    global img_format, brightness, contrast, sharpness, \
        ...
        corruption_amount, jpeg_header_size, selections, layering_system
    ...
    layering_system = request.form.get('layering_system') == 'true'
