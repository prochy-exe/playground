...
select_monitor() {
    get_focused_monitor() {
        ...
        read MOUSE_X MOUSE_Y <<< $(xdotool getmouselocation --shell | grep -E 'X=|Y=' | awk -F '=' '{print $2}')
        ...
        while IFS= read -r line; do
            if [[ $line =~ ^([a-zA-Z0-9-]+)\ connected\ primary\ ([0-9]+)x([0-9]+)\+([0-9]+)\+([0-9]+) ]]; then
                ...
                X=${BASH_REMATCH[4]}
                Y=${BASH_REMATCH[5]}
            elif [[ $line =~ ^([a-zA-Z0-9-]+)\ connected\ ([0-9]+)x([0-9]+)\+([0-9]+)\+([0-9]+) ]]; then
                MONITOR_NAME=${BASH_REMATCH[1]}
                WIDTH=${BASH_REMATCH[2]}
                HEIGHT=${BASH_REMATCH[3]}
                X=${BASH_REMATCH[4]}
    ...
    get_monitor_info() {
        MONITOR_INFO=$(xrandr --listmonitors | grep -i "$1" | awk '{print $3}')
        IFS='/' read -r WIDTH H CORDS <<< "$MONITOR_INFO"
        IFS='x' read -r mmHeight HEIGHT <<< "$H"
        IFS='+' read -r idk X Y <<< "$CORDS"
    }
    ...
    get_monitor_by_index() {
        ...
        INDEX_NAME="$(xrandr --listmonitors | grep "$index:"  | awk '{print $2}' | sed "s/+//" | sed "s/*//")"
    ...
    if [ -z "$1" ]; then
        ...
        MONITORS=$(xrandr --listmonitors | grep -oe '  .*$' | sed -e 's/^[ \t]*//')
        ...
        PRESEL_MONITOR=$1
        re='^[0-9]+$'
        if [[ "$PRESEL_MONITOR" == "primary" ]]; then
            get_monitor_info "$(xrandr --query | grep primary | awk '{print $1}')"
        elif [[ "$PRESEL_MONITOR" == "focused" ]]; then
